...
export const DeleteModal = (props) => {
    ...
    return(
        <Modal open={open}>
            <ModalPaper sx={{p:4}}>
                <Box sx={{mb:2}}>
                    <Typography sx={{fontWeight:"bold", fontSize:"24px"}}>{count}개의 데이터를 삭제하시겠습니까?</Typography>
                    <Typography>삭제되는 데이터는 영구적으로 삭제되고 복구가 불가능합니다.</Typography>
                </Box>
                <Box sx={{display:"flex", justifyContent:"flex-end", width:"auto"}}>
                    <ModalButton sx={{color:"red", mx:"3px"}} onClick={deletePolls}>삭제</ModalButton>
                    <ModalButton sx={{color:"black", mx:"3px"}} onClick={handleModal}>취소</ModalButton>
                </Box>
            </ModalPaper>
        </Modal>
    )
            ...
            <ModalPaper sx={{p:4, width:"400px"}}>
                <Box sx={{mb:2}}>
                    ...
                    <NeumorphismTextField 
                        ...
                        placeholder="프로젝트 이름"
                        InputProps={{
                            endAdornment: <InputAdornment position="end">{25 - projectName.length}</InputAdornment>,
                        }}
                        value={projectName}
                ...
                <Box sx={{display:"flex", justifyContent:"flex-end", width:"auto"}}>
                    <ModalButton sx={{color:"blue", mx:"3px"}} onClick={createProjectName}>생성</ModalButton>
                    <ModalButton sx={{color:"black", mx:"3px"}} onClick={handleModal}>취소</ModalButton>
                </Box>
            </ModalPaper>
