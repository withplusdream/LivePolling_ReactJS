...
const Content = () => {
                ...
                <Grid container spacing={1}>
                    ...
                    <Grid item xs={6}>
                        <Tooltip title="시작" arrow>
                            <ActionButton id={params.id} size="small" onClick={goSlidePage}>
    ...
    const goSlidePage = (e) => {
        navigate(`/manager/app/${e.currentTarget.id}`)
    }
    ...
    useEffect(() => {
        ...
        socket.on("getProjectsByAdmin", (data) => {
            ...
            for(var i=0; i<data.length; i++){
                newProjects.push({id:data[i]._id, name: data[i].name, type: data[i].type, created: data[i].created.slice(0,10)})
            }
    ...
    return (
        <Box sx={{width:"80%", p:4, boxSizing:"border-box"}}>
            ...
            <DeleteModal open={removePollModalIsOpened} handleModal={handleRemovePollModal} count={checkedList.length} deletePolls={deletePolls}/>
